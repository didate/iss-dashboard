...
export default function GeoLabels({ features, text, minZoom = 0 }: Props) {
  const map = useMap();
      ...
      try {
        ...
        const icon = L.divIcon({
          ...
          html: `<div style="text-align:center;pointer-events:none;text-shadow:1px 1px 2px white,-1px -1px 2px white,1px -1px 2px white,-1px 1px 2px white;">
            <div style="font-size:10px;font-weight:700;color:#1f2937;line-height:1.2;">${f.properties.name}</div>
            <div style="font-size:11px;font-weight:800;color:#1e3a5f;line-height:1.2;">${text(f.properties)}</div>
          </div>`,
          iconSize: [90, 30],
          iconAnchor: [45, 15],
        });
