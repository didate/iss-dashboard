...
1. **Table de correspondance** `data/DRH - correspondances structures.csv` — les cas validés à
   ...
   L'unicite porte sur le couple **(libelle, district)**, pas sur le libelle seul : « HRK » designe l'hopital
   regional de Kankan a Kankan et celui de Kindia a Kindia, et les deux regles coexistent. A cle egale, **la
   derniere ligne gagne** : la table s'edite en ajoutant a la fin, donc une correction ecrite apres coup
   l'emporte sur la regle qu'elle corrige.
